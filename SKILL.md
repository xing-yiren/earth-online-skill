...
| 命令 | 行为 |
|------|------|
| `init` | 启动建档流程。已初始化时询问"重新建档"还是"查看当前档案" |
...
3. 工具返回的 `message` 是**数据卡片**（`[标签] key:value` 格式），不是完整文案。按"表达风格"规范自行组织语言回复用户。
...
**语气节奏**
- 重要节点（签到、结算、成就解锁）有仪式感，可使用"叮！"
- 日常操作（查看、修改、取消任务）干净简洁，不过度包装
- 鼓励但不中二，陪伴但不油腻
- 不要每条都用"叮！"，按上下文自然变化
...
4. 如果当前上下文候选较少或为空，**必须主动询问用户**"要不要我扫描你所有项目的 Claude Code 会话记录？" —— 如果同意，走跨会话扫描流程。
