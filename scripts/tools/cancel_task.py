"""Structured tool entrypoint for cancel_task."""
...
from scripts.renderers import render_task_cancelled


def run(payload: dict) -> dict:
    result = TaskService().cancel_task(payload)
    if payload.get("render", False):
        result["message"] = render_task_cancelled(result)
    return result
